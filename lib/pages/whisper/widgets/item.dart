...
class WhisperSessionItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // 会话操作（置顶 / 标为已读 / 免打扰 / 删除）：触摸长按、鼠标右键都用它，
    // 手柄那边是 Y 键 / 遥控器菜单键（见下面的 `TvKeys.isMore`）
    void showActionsDialog() => showDialog(
      context: context,
    ...
    return FocusRing(
      // 描边跟着整行的高度走（ListTile 自己那点 focus 高亮手柄上看不出来）
      radius: const .all(.circular(12)),
      debugLabel: '会话',
      onKeyEvent: (node, event) {
        // Y / 遥控器菜单键 = 会话操作（触摸那边是长按）。
        // 不做长按确定：整行只有一个动作，长按再弹菜单反而多一步。
        if (!Pref.tvFocus || !TvKeys.isMore(event)) {
          return KeyEventResult.ignored;
        }
        if (TvKeys.isFirstPress(event)) showActionsDialog();
        return KeyEventResult.handled;
      },
      builder: (context, node, _) => ListTile(
        focusNode: node,
          ...
          children: [
            ...
            if (item.isMuted)
              Icon(
                ...
                item.unread.style != .UNREAD_STYLE_NONE)
              Badge(
